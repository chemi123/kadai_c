...
/*
 ...
 時間計算量はO(N^2*days)になる。ノードの探索なので一見Nに線形になりそうだと思ったが、実測してN^2であることに気づいた。理由としてはキャッシュの探索でさらにNだけ時間がかかっているのではないかと考えられる(もっと速くなる？)。
 測り方は粗い上マシンパワーにも依存するが以下は実測データ。(方針1と方針2で実行結果の差分がないことは確認している)。
...
func collectMap2(schedules [][]int, maxDay int) int {
	...
	for day := 1; day <= maxDay; day++ {
		...
		for _, node := range nodesList[day] {
			...
			if node.left != nil {
				// ここで遅くなっているので、mapの差分だけ取得するいい方法がないのか気になる
